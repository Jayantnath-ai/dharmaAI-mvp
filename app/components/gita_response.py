...
from components.modes import (
    generate_arjuna_reflections,
    generate_dharma_mirror_reflections,
)
...
def generate_gita_response(mode, df_matrix, user_input=None):
    ...
    if df_matrix is not None and not df_matrix.empty:
        if "embedding" not in df_matrix.columns:
            df_matrix["embedding"] = df_matrix["Short English Translation"].apply(
                lambda x: get_embedding(str(x))
            )
        user_embedding = get_embedding(user_input)
        df_matrix["similarity"] = df_matrix["embedding"].apply(
            lambda emb: cosine_similarity(user_embedding, emb)
        )
        ...
        if mode == "Krishna-Explains":
            ...
            if os.getenv("OPENAI_API_KEY"):
                ...
                response = (
                    f"**🤖 Krishna-Explains (offline mode):**\n\n"
                    f"_Reflecting on your question:_ **{user_input}**\n\n"
                    f"> {verse_info['Short English Translation'] if verse_info is not None else '[Simulated GPT response]'}"
                )
        elif mode == "Krishna":
            response = (
                f"**🧠 Krishna teaches:**\n\n"
            ...
            reflection_text = "\n".join(
                [f"{idx+1}. {line}" for idx, line in enumerate(reflections)]
            )
            response = (
            ...
            reflections, matched_verse = generate_dharma_mirror_reflections(
                user_input, df_matrix
            )
            reflection_text = "\n".join(
                [f"{idx+1}. {line}" for idx, line in enumerate(reflections)]
            )
            ...
            similarity_score = (
                verse_info["similarity"]
                if verse_info is not None and "similarity" in verse_info
                else "N/A"
            )
            response = (
                f"**📖 Vyasa Narrates:**\n\n"
