...
def generate_gita_response(mode, df_matrix, user_input=None):
    ...
    if mode == "Krishna-GPT":
        ...
        try:
            ...
            if gemini_api_key:
                ...
                model_name = selected_gemini_model, "models/gemini-1.5-flash"
        ...
        similarity_score = float(verse_info['similarity']) if verse_info is not None and 'similarity' in verse_info and pd.notnull(verse_info['similarity']) else 'N/A'
        response = (
            f"**📖 Vyasa Narrates:**\n\n"
    ...
    if streamlit_available:
        ...
        import json
        with open("usage_journal_log.json", "w") as f:
            json.dump(st.session_state["Usage Journal"], f, indent=2)
...
if streamlit_available:
    ...
    if st.button("🔍 Submit"):
        if df_matrix is None or df_matrix.empty:
            ...
            st.markdown("To debug, ensure the CSV is present in `/data/` or root folder in your GitHub repo.")
            st.stop()
        try:
    ...
    if "Usage Journal" in st.session_state and st.session_state["Usage Journal"]:
        with st.expander("🕰️ View Past Interactions"):
            ...
            import base64
            import json
            journal_json = json.dumps(st.session_state["Usage Journal"], indent=2)
            b64 = base64.b64encode(journal_json.encode()).decode()
            href = f'<a href="data:file/json;base64,{b64}" download="usage_journal_log.json">📥 Download session log (JSON)</a>'
